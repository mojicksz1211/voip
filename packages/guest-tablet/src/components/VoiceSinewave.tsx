...
/**
 * Live audio waveform that reacts to the remote party's voice. It taps every
 * <audio> element on the page (where the WebRTC/LiveKit remote track is played)
 * and renders its real-time amplitude as animated sine waves.
 */
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const waves: { color: string; alpha: number; phase: number; freq: number }[] = paused
        ? [
            { color: '#94a3b8', alpha: 0.4, phase: 0, freq: 3 },
            { color: '#cbd5e1', alpha: 0.28, phase: 1.2, freq: 2.2 },
          ]
        : [
            { color: '#60a5fa', alpha: 0.95, phase: 0, freq: 4 },
            { color: '#34d399', alpha: 0.7, phase: 0.9, freq: 5.2 },
            { color: '#a78bfa', alpha: 0.5, phase: 1.8, freq: 3.1 },
          ];
